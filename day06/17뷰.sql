--뷰
--뷰는 제한적인 데이터를 쉽게 보기위해서 미리 만들어 놓은 가상(가짜) 테이블
--자주 사용되는 컬럼을 저장하면 관리가 용이
--뷰는 물리적으로 데이터가 저장된 것은 아니고 원본 테이블을 기반으로 한 가상테이블
--뷰를 만드려면 권한이 필요함 (HR) 권한을 가지고 있음
...
SELECT * FROM VIEW_EMP;
--NAME, EMP_ID가 가상열이기 때문이 INSERT가 들어가지 못함
INSERT INTO VIEW_EMP VALUES(108, 'HONG', 'IT_PROG', 10000);
--원본 테이블의 NOT NULL제약에 위배되기 때문에 들어가지 못함
INSERT INTO VIE_EMP (JOB_ID, SALARY) VALUES('IT_PROG', 10000);

--뷰의 옵션
--WITH CHECK OPTION (WHERE절에 있는 컬럼의 변경을 금지)
