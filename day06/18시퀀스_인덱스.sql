...
--시퀀스 사용방법 2개
SELECT DEPTS_SEQ.CURRVAL FROM DUAL; -- 현재 시퀀스 (NEXTVAR가 선행 되어야함)
...
--만약 시퀀스를 초기화 해야한다면?
--시퀀스의 증가값을 -음수로 만들어서 초기화 인것처럼 쓸 수는 있음
...
--INDEX
--INDEX는 PK, UNIQUE에 자동으로 생서되고, 조회를 빠르게 하는 HINT 역할을 함
--INDEX 종류는 고유인덱스, 비고유인덱스 가 있음
--UNIQUE한 컬럼에는 UNIQUE인덱스(고유) 인덱스가 쓰임
--일반컬럼에는 비고유인덱스를 지정할 수 있음
--INDEX는 조회를 빠르게 하지만, DML구문이 많이 사용되는 컬럼은 오히려 성능저하를 부를 수 있음
