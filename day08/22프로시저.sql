--스토어드 프로시저 (일련의 SQL 처리 과정을 집합처럼 묶어 사용하는 구조)
...
IS
BEGIN
    dbms_output.put_line('HEELO WORLD');
END;
--호출
EXEC NEW_JOB_POC;
...
EXEC NEW_JOB_PROC('SAMPLE', 'SAMPLE2'); -- DEFAULT 매개변수가 있다면 기본값으로 전달됨
...
BEGIN
    ...
    dbms_output.put_line('프로시저 내부에서 할당 받은 값 : ' || CNT);
END;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('프로그램 정상 종료!!');
